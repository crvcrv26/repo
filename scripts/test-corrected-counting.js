...
const testCorrectedCounting = async () => {
  try {
    ...
    console.log(`\n📅 Testing corrected user count for ${year}-${String(monthNum).padStart(2, '0')}`);
    console.log(`   Start of month: ${startOfMonth.toISOString()}`);
    console.log(`   End of month: ${endOfMonth.toISOString()}`);

    // Corrected logic: Count ALL users (all roles) - active + deleted in this month
    ...
    console.log('\n📊 Corrected counting logic results:');
    ...
    console.log(`\n🎯 Final result: ${totalUserCount} users (${activeUsersAtEndOfMonth} active + ${usersDeletedInThisMonth} deleted in this month)`);
